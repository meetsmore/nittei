...
CREATE INDEX IF NOT EXISTS events_parent_id ON calendar_events (parent_id);
...
COMMENT ON TABLE event_reminder_versions IS 'There are three usecases which can generate event reminders.
...
COMMENT ON COLUMN reminders.identifier IS 'User defined identifier to be able to seperate reminders at same timestamp for
    the same event';
...
CREATE TABLE IF NOT EXISTS service_users (
    ...
    user_uid uuid NOT NULL REFERENCES users(user_uid) ON DELETE CASCADE,
    available_calendar_uid uuid REFERENCES calendars(calendar_uid) ON DELETE
    SET
        NULL,
        available_schedule_uid uuid REFERENCES schedules(schedule_uid) ON DELETE
    SET
        NULL,
        ...
        CHECK (
            NOT (
                available_calendar_uid IS NOT NULL
                AND available_schedule_uid IS NOT NULL
            )
        )
);
