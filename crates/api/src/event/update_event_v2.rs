...
use nittei_api_structs::update_event_v2::*;
use nittei_domain::{
    Account,
    CalendarEvent,
...
#[utoipa::path(
    put,
    tag = "Event",
    path = "/api/v1/user/events_v2/{event_id}",
    summary = "Update an event (admin only) (V2)",
    params(
    ...
    request_body(
        content = UpdateEventRequestBodyV2,
    ),
    responses(
        (status = 200, body = APIResponse)
    )
)]
pub async fn update_event_admin_controller_v2(
    Extension(account): Extension<Account>,
    Extension(event): Extension<CalendarEvent>,
    Extension(ctx): Extension<NitteiContext>,
    body: Valid<Json<UpdateEventRequestBodyV2>>,
) -> Result<Json<APIResponse>, NitteiError> {
    let user = account_can_modify_user(&account, &event.user_id, &ctx).await?;

    let mut body = body.0;
    let usecase = UpdateEventUseCaseV2 {
        user,
...
#[utoipa::path(
    put,
    tag = "Event",
    path = "/api/v1/events_v2/{event_id}",
    summary = "Update an event (user only) (V2)",
    params(
        ("event_id" = ID, Path, description = "The id of the event to update"),
    ),
    request_body(
...
pub async fn update_event_controller_v2(
...
pub struct UpdateEventUseCaseV2 {
    pub user: User,
    pub event_id: ID,
...
impl UseCase for UpdateEventUseCaseV2 {
...
impl PermissionBoundary for UpdateEventUseCaseV2 {
...
mod test {
    ...
    async fn update_nonexisting_event() {
        let mut usecase = UpdateEventUseCaseV2 {
